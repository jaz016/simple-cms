<?php

    session_start();
    include 'config.php';
    include 'classes/autoloader.php';
    include 'inc/connection.php';
    include 'inc/user_init.php';

    $portfolios = new Portfolio();
    $portfolio_items = $portfolios->fetch_all();
    $portfolio = Portfolio::fetch_one($_GET['id']);
    $page_title = $portfolio->get_title();

?>



<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="">
    <meta name="author" content="">

    <title><?php echo $page_title . " | " . SITE_TITLE; ?></title>

    <!-- Bootstrap Core CSS -->
    <link href="<?php echo ROOT_DIR; ?>/assets/css/bootstrap.min.css" rel="stylesheet">

    <!-- Custom CSS -->
    <link href="<?php echo ROOT_DIR; ?>/assets/css/modern-business.css" rel="stylesheet">

    <!-- Custom Fonts -->
    <link href="<?php echo ROOT_DIR; ?>/assets/font-awesome/css/font-awesome.min.css" rel="stylesheet" type="text/css">

    <!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
        <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
        <script src="https://oss.maxcdn.com/libs/respond.js/1.4.2/respond.min.js"></script>
    <![endif]-->

</head>

<body>

    <!-- Navigation -->
    <?php include 'layout/navbar.php'; ?>

    <!-- Page Content -->
    <div class="container">

        <!-- Page Heading/Breadcrumbs -->
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header"><?php echo $portfolio->get_title(); ?>
                </h1>
                <ol class="breadcrumb">
                    <li><a href="<?php echo ROOT_DIR; ?>">Home</a>
                    </li>
                    <li class="active"><?php echo $portfolio->get_title(); ?></li>
                </ol>
            </div>
        </div>
        <!-- /.row -->

        <!-- Portfolio Item Row -->
        <div class="row">

            <div class="col-md-8">
                <img class="img-responsive" src="/oop-cms/uploads/<?php echo $portfolio->get_image(); ?>" alt="">
            </div>

            <div class="col-md-4">
                <h3>Project Description</h3>
                <p><?php echo $portfolio->get_content(); ?></p>

                <hr>


                <strong>Date Posted: </strong>
                <span><?php echo $portfolio->get_date_posted(); ?></span>
            </div>

        </div>
        <!-- /.row -->

        <!-- Related Projects Row -->
        <div class="row">

            <div class="col-lg-12">
                <h3 class="page-header">Related Projects</h3>
            </div>


            <?php
            $i = 0;
            foreach ($portfolio_items as $portfolio) :
            ?>


            <div class="col-sm-3 col-xs-6">
                <a href="portfolio-item.php?id=<?php echo $portfolio->get_id(); ?>">
                    <div class="image-wrap" style="height: 150px;overflow: hidden;border: 1px solid #f5f5f5;">
                        <img class="img-responsive img-hover img-related" src="/oop-cms/uploads/<?php echo $portfolio->get_image(); ?>" alt="">
                    </div>
                </a>
            </div>

            <?php
            if($i == 3)
                break;
            $i++;
            endforeach;
            ?>

        </div>
        <!-- /.row -->

        <hr>

        <!-- Footer -->
        <footer>
            <div class="row">
                <div class="col-lg-12">
                    <p>Copyright &copy; Your Website 2014</p>
                </div>
            </div>
        </footer>

    </div>
    <!-- /.container -->

    <!-- jQuery -->
    <script src="js/jquery.js"></script>

    <!-- Bootstrap Core JavaScript -->
    <script src="js/bootstrap.min.js"></script>

</body>

</html>
